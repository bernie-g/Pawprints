...
public class EntityBilby extends EntityAnimal implements IAnimatedEntity
{	
	...
	private <E extends EntityBilby> boolean animationPredicate(AnimationTestEvent<E> event)
	{
		if(event.isWalking())
		{
			controller.setAnimation(new AnimationBuilder().addAnimation("animation.ModelBilbyfix.walk").addAnimation("animation.ModelBilbyfix.walk", true));
			return true;
		} else {
			controller.setAnimation(new AnimationBuilder().addAnimation("animation.ModelBilbyfix.idle", true));
			return true;
		}
	}
